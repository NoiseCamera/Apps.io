...
export const MakeupEditor: React.FC<MakeupEditorProps> = ({ imageSrc, onApply, onCancel }) => {
  ...
  const updateBrushTip = useCallback(() => {
      ...
      const hardness = activeBrush === 'liner' ? 0.8 : 0.0;
  ...
  const setToolDefaults = (type: BrushType) => {
      setActiveBrush(type);
      switch(type) {
          case 'lip': setBrushColor('#d946ef'); setBrushSize(30); setBrushOpacity(50); break;
          case 'blush': setBrushColor('#fda4af'); setBrushSize(80); setBrushOpacity(20); break;
          case 'eye': setBrushColor('#a855f7'); setBrushSize(40); setBrushOpacity(30); break;
          case 'wig': setBrushColor('#3b82f6'); setBrushSize(100); setBrushOpacity(40); break;
          case 'contact': setBrushColor('#22d3ee'); setBrushSize(20); setBrushOpacity(60); break;
          case 'liner': setBrushColor('#000000'); setBrushSize(5); setBrushOpacity(80); break;
          case 'eraser': setBrushSize(40); setBrushOpacity(100); break;
          case 'blur': setBrushSize(50); setBrushOpacity(50); break;
      }
  };
  ...
  const drawStroke = (startX: number, startY: number, endX: number, endY: number) => {
      ...
      if (activeBrush === 'eraser') {
          ...
          if (activeBrush === 'wig') ctx.globalCompositeOperation = 'overlay';
      ...
      if (steps > 0) ctx.drawImage(tip, endX - offset, endY - offset);
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
      setCursorPos({ x: e.clientX, y: e.clientY }); // Always update cursor
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
      ...
      if (pointersRef.current.size === 0) {
          ...
          isDrawing.current = false;
          // setCursorPos(null); // Keep cursor for visibility
  ...
  // Calculate visible cursor size (scaled by zoom)
  const visibleBrushSize = cursorPos ? brushSize * transform.scale : 0;

  const colors = ['#ec4899', '#ef4444', '#3b82f6', '#22c55e', '#eab308', '#a855f7', '#06b6d4', '#f97316', '#000000', '#ffffff'];
    ...
    <div className="flex flex-col h-full bg-cos-dark/90 backdrop-blur-xl z-50 absolute inset-0">
       ...
       <div className="h-14 px-4 flex items-center justify-between border-b border-white/10 bg-black/50 z-10 shrink-0">
        ...
        <div className="flex-1 relative overflow-hidden touch-none flex items-center justify-center bg-black">
            ...
            {cursorPos && (
                <div 
                    className="absolute rounded-full border border-white/80 bg-white/10 pointer-events-none z-50 transform -translate-x-1/2 -translate-y-1/2 mix-blend-difference"
                    style={{ 
                        left: cursorPos.x, 
                        top: cursorPos.y, 
                        width: visibleBrushSize, 
                        height: visibleBrushSize 
                    }}
                />
            )}
            ...
            <div className="max-w-lg mx-auto space-y-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                     <Slider 
                        ...
                        onReset={() => setBrushSize(30)} 
                     />
                     <Slider 
                        label={activeBrush === 'blur' ? '強さ' : '濃さ'} 
                        value={brushOpacity} 
                        min={1} 
                        max={100} 
                        onChange={setBrushOpacity}
                        unit="%"
                        onReset={() => setBrushOpacity(50)}
                     />
                </div>
