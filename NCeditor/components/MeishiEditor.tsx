...
interface MeishiEditorProps {
  ...
  onChange: (state: MeishiState) => void;
  onClose: () => void;
  onEditImage: () => void;
}
...
type DragTarget = 'photo' | 'name' | 'details' | 'qr' | 'none';
type DragAction = 'move' | 'scale';
type EditorTab = 'templates' | 'content' | 'style' | 'arrange';
...
export const MeishiEditor: React.FC<MeishiEditorProps> = ({ imageSrc, initialState, onChange, onClose, onEditImage }) => {
  ...
  const initialElPos = useRef<ElementPos | null>(null); 
  
  // To store calculated bounds for hit testing
  const elementBounds = useRef<Map<DragTarget, { x: number, y: number, w: number, h: number, angle: number }>>(new Map());
  ...
  const draw = (ctx: CanvasRenderingContext2D, img: HTMLImageElement, exportMode: boolean = false) => {
      ...
      if (initialState.namePos.visible) {
          ...
          ctx.textAlign = (namePos.align as CanvasTextAlign) || 'center';
      ...
      if (initialState.detailsPos.visible) {
          ...
          ctx.font = `${weight} ${size}px ${fontCSS}`;
          ctx.textAlign = (detailsPos.align as CanvasTextAlign) || 'center';
          ctx.textBaseline = 'middle';
  ...
  const drawFrame = (ctx: CanvasRenderingContext2D, w: number, h: number, style: FrameStyle, color: string, sub: string) => {
  ...
  const drawSelection = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) => {
  ...
  const getPointerLocal = (e: React.PointerEvent, canvas: HTMLCanvasElement) => {
  ...
  const checkHandleHit = (px: number, py: number, target: DragTarget, align?: string): boolean => {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
      ...
      const targets: DragTarget[] = ['qr', 'details', 'name', 'photo'];
          ...
          if (asA4) {
              ...
              if (ctx && loadedImg) {
                  ...
                  for (let i = 0; i <= 2; i++) {
                      ...
                      ctx.beginPath();
                      ctx.moveTo(x, Math.max(0, startY - 100)); 
                      ctx.lineTo(x, Math.min(a4Canvas.height, startY + cardH * 5 + 100));
                      ctx.stroke();
                  }
                  ...
                  for (let i = 0; i <= 5; i++) {
                      ...
                      ctx.beginPath();
                      ctx.moveTo(Math.max(0, startX - 100), y);
                      ctx.lineTo(Math.min(a4Canvas.width, startX + cardW * 2 + 100), y);
                      ctx.stroke();
                  }
  ...
  const updateAlign = (target: 'name' | 'details', align: 'left' | 'center' | 'right') => {
