...
const FilterMenu = ({ activePresetId, onSelect, onClose }) => {
    const scrollRef = useRef(null);

    useEffect(() => {
        // Scroll to active item on mount
        if (scrollRef.current) {
            const activeEl = scrollRef.current.querySelector(`button[data-active="true"]`);
            if (activeEl) {
...
export const Camera = ({ onCapture, onClose }) => {
    ...
    const [activePresetId, setActivePresetId] = useState('fp-none');
