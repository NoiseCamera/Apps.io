...
export const MaskEditor: React.FC<MaskEditorProps> = ({ imageSrc, initialMask, onSave, onCancel }) => {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
      ...
      if (pointersRef.current.size === 1) {
          if (activeTool === 'magic') {
              ...
              lastPos.current = getPointerPos(e.clientX, e.clientY);
          } else {
  ...
  // Visible Brush Size calculation
    ...
    <div className="flex flex-col h-full bg-cos-dark/80 backdrop-blur-xl z-50 absolute inset-0">
        ...
        <div className="flex-1 relative overflow-hidden touch-none flex items-center justify-center bg-black">
            ...
            >
                ...
                <canvas 
                    ref={canvasRef}
                    className="max-w-none max-h-none object-contain cursor-none relative z-10 opacity-60" 
                    style={{
                        maxWidth: '100%',
                        maxHeight: '100%'
                    }}
            ...
            {/* Brush Cursor */}
            {cursorPos && activeTool !== 'magic' && (
                <div 
                    className="absolute rounded-full border-2 border-white/80 pointer-events-none z-50 transform -translate-x-1/2 -translate-y-1/2 shadow-lg"
                    style={{ 
                        left: cursorPos.x, 
                        top: cursorPos.y, 
            ...
            <div className="max-w-lg mx-auto space-y-4">
                ...
                {activeTool !== 'magic' && (
                    <div className="grid grid-cols-3 gap-4 animate-fade-in">
                        <Slider label="サイズ" value={brushSize} min={5} max={150} onChange={setBrushSize} onReset={() => setBrushSize(50)} />
                        <Slider label="ぼかし" value={100 - brushHardness} min={0} max={100} onChange={(v) => setBrushHardness(100 - v)} onReset={() => setBrushHardness(80)} />
                        <Slider label="濃度" value={brushOpacity} min={10} max={100} onChange={setBrushOpacity} onReset={() => setBrushOpacity(100)} unit="%" />
                    </div>
                )}
