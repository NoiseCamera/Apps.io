...
import { generateAIMask, generateAIMaskFromPoint } from '../services/geminiService';

export const MaskEditor = ({ imageSrc, initialMask, onSave, onCancel }) => {
    ...
    const startPos = useRef(null); // To detect taps vs drags
    ...
    const applyAiMask = async (maskDataUrl) => {
        const maskImg = new Image();
    ...
    const handleAutoMask = async (target) => {
    ...
    const handlePointMask = async (x, y) => {
        if (!imageSrc || isAiLoading) return;
