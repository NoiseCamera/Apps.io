...
export type MaskTarget = 'person' | 'background' | 'hair' | 'face' | 'clothes' | 'skin' | 'face_only';

export const generateAIMask = async (imageBase64: string, target: MaskTarget = 'person'): Promise<string> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const model = "gemini-2.5-flash-image";
...
export const generateAIMaskFromPoint = async (imageBase64: string, x: number, y: number): Promise<string> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const model = "gemini-2.5-flash-image";
...
export const generateCosplayCaption = async (imageBase64: string): Promise<AICaptionResult> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        ...
        return JSON.parse(text) as AICaptionResult;
